<?php
namespace Opencart\Catalog\Controller\Freshworks;
...
class Info extends \Opencart\System\Engine\Controller
{
	...
	protected function getOrderInfo($email)
	{
		...
		foreach ($orderList as $order) {
			...
			$totals = $this->model_account_order->getTotals($order['order_id']);
			$products = $this->model_account_order->getProducts($order['order_id']);
			$vouchers = $this->model_account_order->getVouchers($order['order_id']);
			$totalsFormatted = [];
			...
			foreach ($products as $product) {
				$options = $this->model_account_order->getOptions($order['order_id'], $product['order_product_id']);
				$optionList = [];
